...
export default function BookingCard({ booking }) {
  ...
  const canShowRating = s === "completed" && hasRating
  const needsReview = s === "completed" && !hasRating

  const [copied, setCopied] = useState(false)

  const copyReviewLink = async () => {
    try {
      const url = `${window.location.origin}/review/${d._id}`
  ...
  return (
    <div
      className={`rounded-lg border border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md border-t-4 ${statusBorderClass(d.status)}`}
    >
      {/* Header */}
        ...
        <dl className="space-y-2">
          <div className="flex justify-between">
            ...
            <dd className="font-medium text-slate-800 text-right">
              {d.carDetails?.make} {d.carDetails?.model}
              {d.carDetails?.type ? ` (${d.carDetails.type})` : ""}
            </dd>
          </div>
          <div className="flex justify-between">
            ...
            <dd className="text-right">
              {canShowRating ? (
                <div className="inline-flex items-end">
                  <RatingStars value={d.rating || 0} readOnly />
                </div>
              ) : (
      ...
      {/* Footer & Actions */}
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-b-md bg-slate-50/75 p-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {needsReview && (
            <button
              ...
              className="rounded-md border border-emerald-300 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700 transition-colors hover:bg-emerald-100"
              aria-label="Copy shareable review link"
            >
